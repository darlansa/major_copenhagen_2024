...
load_dotenv()
def get_head():
    head = {'Authorization':'Bearer ' + dotenv_values()['ACESS_TOKEN'],'Client-Id':dotenv_values()['CLIENT_ID']}
    return head
...
def analyser_stream_view(id_stream,viewer_count):
    return con.execute(f'SELECT id_stream from views_stream where id_stream = {id_stream} and viewer_count = {viewer_count}').fetchone()
...
if validate_token().status_code != '200':
    ...
    for streamer in streamers['data']:
        ...
        if not analyser_streamer(user_id=user_id_streamer):
            insert_streamer(language=language_streamer, user_id=user_id_streamer,user_name=user_name_streamer)
        
        if not analyser_stream(id=id_stream):
            insert_stream(id=id_stream,id_streamer=user_id_streamer, started_at=started_at_stream, title=title_stream)
        
        if not analyser_stream_view(id_stream=id_stream, viewer_count=viewer_count_stream):
